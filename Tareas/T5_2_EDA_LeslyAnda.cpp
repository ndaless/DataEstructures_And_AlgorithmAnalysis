# include <iostream>
# include <vector>
#include <deque>

using namespace std;

// template <typename nodeVal> 
class KSJ{
    int N;

    vector <int> vals;
    vector <int> special;
    vector< vector<int> > edges;
    vector< vector<int> > segde;
    deque<int> L; // guardamos el indice de nuestro nodo
    vector <int> visAssg;
    vector< vector<int> > CC; // Componentes Conexas

public: 

    KSJ(vector <int> _vals, vector< vector<int> > _edges, vector< vector<int> > _segde, vector <int> _special){
        N = _edges.size();

        edges = _edges;
        segde = _segde;
        vals = _vals;
        special = _special;
        
        CC.resize(N);
        
        // L.resize(N); // ya no es necesaria pq usaremos una deque
        visAssg.assign(N, 0);
        
        algKSJ();
    } 

void visit(int nd){
    // cout << "entrando a visit" << endl;

    visAssg[nd] = 1;

    for(const auto child:edges[nd]){
        if(visAssg[child] != 1) visit(child);
    }

    L.push_front(nd);

    return;
}

void assign(int nd, int root){
    // cout << "entrnado a assign" << endl;

    if(visAssg[nd] != 1){
        CC[root].push_back(nd);
        visAssg[nd] = 1;
    }
    
    for(const auto child : edges[nd]){
        if(visAssg[child] != 1){
            assign(child, root);
        }
    }

}

void algKSJ(){
    // cout << "entrando a algKSJ" << endl;
    for(int nd = 0; nd < N; nd++){
        if(visAssg[nd] != 1){
            visit(nd);
        }
    }

    visAssg.assign(N, 0); 
    int comp = 0; int nd;

    while( !L.empty() ){

        nd = L.back(); // cout << nd << endl;
        // cout << segde[nd].empty() << endl;
        if(visAssg[nd] != 1){
            // cout << "creando cc numero: " << nd << endl; 
            /// vector< int > root; root.push_back(nd);
            // CC.push_back(root);
            assign(nd, nd);
        } 
        // cout << "entrando a while" << endl;
        L.pop_front(); // cout << L.front() << endl; 
    }

    return;
}

int ansQry(){ // maybe i dont think so, but maybe a long long
    // cout << "entrando a ansQry" << endl;
    int magicBoxes = 0;
    
    for(const auto c : CC){
        if(c.size() > 1 ) magicBoxes += c.size();
        else if(c.size()==1 && special[c[0]] == 1) magicBoxes++;
    }
    
    // for(const auto c : CC){
    //     for(const auto nd : c){
    //         cout << nd;
    //     }
    //     cout << endl;
    // }
    
    return magicBoxes;
}

};

int main(){
    // leer grafo y al mismo tiempo crear grafo inverso
    int T, N; cin >> T;

    int jump, destination;
    
    for(int t = 0; t<T; t++){
        cin >> N;
        vector <int> vals(N);
        vector <int> special(N, 0);
        vector< vector<int> > edges(N);
        vector< vector<int> > segde(N);
        
        for(int i = 0; i < N; i++){
            cin >> jump; 
            vals[i] = jump;
            destination = (i+jump)%N;
            edges[i].push_back(destination);
            segde[destination].push_back(i);
            if(i == destination) special[i] = 1;
            // cout << "aristas leidas" << endl;
        }
        
        KSJ myKSJ(vals, edges, segde, special);
        cout << myKSJ.ansQry() << endl;
    }
    
    return 0;
}
